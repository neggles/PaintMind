...
def mask_schedule(ratio):
    return np.cos(math.pi / 2.0 * ratio)
...
class Pipeline(nn.Module):
    def __init__(self, config, stage1_pretrained=True, stage1_checkpoint_path=None):
        super().__init__()
        t5_version = {
            "t5-l": "google/flan-t5-large",
            "t5-xl": "google/flan-t5-xl",
            "t5-xxl": "google/flan-t5-xxl",
        }
        t5_txt_dim = {"t5-l": 1024, "t5-xl": 2048}

        self.vqgan = pm.create_model(
            arch="vqgan",
            version=config.stage1,
            pretrained=stage1_pretrained,
    ...
    def generate(self, text, timesteps=18, temperature=1.0, topk=5, save_interval=2):
        ...
        text = self.text_model(text)
        ids = torch.full(
            (B, self.num_tokens), self.mask_token_id, dtype=torch.long, device=self.mask_token.device
        )
