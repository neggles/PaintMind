...
class VQGANTrainer(nn.Module):
    def __init__(
        ...
        self.train_dl = DataLoader(
            self.train_ds, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=pin_memory
        )
        ...
        self.g_sched = build_scheduler(
            self.g_optim, num_epoch, len(self.train_dl), lr_min, warmup_steps, warmup_lr_init, decay_steps
        )
        self.d_sched = build_scheduler(
            self.d_optim, num_epoch, len(self.train_dl), lr_min, warmup_steps, warmup_lr_init, decay_steps
        )
    ...
    def train(self):
        ...
        for epoch in range(self.num_epoch):
            with tqdm(
                self.train_dl, dynamic_ncols=True, disable=not self.accelerator.is_main_process
            ) as train_dl:
    ...
    def evaluate(self):
        self.vqvae.eval()
        with tqdm(
            self.valid_dl, dynamic_ncols=True, disable=not self.accelerator.is_local_main_process
        ) as valid_dl:
...
class PaintMindTrainer(nn.Module):
    def __init__(
        ...
        self.valid_dl = DataLoader(
            self.valid_ds, batch_size=6, shuffle=False, num_workers=num_workers, pin_memory=pin_memory
        )
        ...
        self.scheduler = build_scheduler(
            self.optim, num_epoch, len(self.train_dl), lr_min, warmup_steps, warmup_lr_init, decay_steps
        )
        ...
        (
            ...
            self.train_dl,
            self.valid_dl,
        ) = self.accelerator.prepare(self.model, self.optim, self.scheduler, self.train_dl, self.valid_dl)
    ...
    def save(self):
        ...
        self.accelerator.save(
            state_dict, os.path.join(self.model_saved_dir, f"paintmind_step_{self.steps}.pt")
        )
    ...
    def evaluate(self):
        self.model.eval()
        with tqdm(
            self.valid_dl, dynamic_ncols=True, disable=not self.accelerator.is_main_process
        ) as valid_dl:
            for i, batch in enumerate(valid_dl):
                imgs, text = batch
                ...
                with self.accelerator.autocast():
                    gens = self.model.generate(
                        text=text, timesteps=18, temperature=1.0, topk=5, save_interval=2
                    )
